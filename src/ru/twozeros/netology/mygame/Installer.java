...
public class Installer {
    ...
    private final String FOLDER_EXISTS = "Каталог не был создан, уже существует";
    ...
    public void createFile(String... fileNames) {
        Arrays.stream(fileNames).map(File::new).forEach(file -> {
            try {
                if (file.createNewFile()) logger.addToLog(STR_SUCCESS, file.getName(), file.getCanonicalPath());
                else if (file.exists()) logger.addToLog(STR_SUCCESS, file.getName(), file.getCanonicalPath());
    ...
    public void createFolder(String... folderNames) {
        Arrays.stream(folderNames).map(File::new).forEach(file -> {
            try {
